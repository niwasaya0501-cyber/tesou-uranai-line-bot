...
5. サーバー側でsharpにより最終リサイズ（長辺768px・EXIF除去）→ OpenAI Visionで鑑定 → LINE Push Messageで結果を送信（鑑定に成功した場合は、続けて「気になることがあれば質問してください」という案内メッセージと、「続きを話す／新しく占う」の2択のクイックリプライも送る）
...
- 鑑定結果のあとの案内メッセージには「続きを話す」「新しく占う」の2択のクイックリプライが付いており、タップするだけで選べます（もちろん自由にメッセージを入力してラリーを始めてもOKです）。「続きを話す」をタップすると「続きを聞きたいです」という定型文が送られ、通常のラリーとして処理されます。「新しく占う」はLIFFを直接開きます
