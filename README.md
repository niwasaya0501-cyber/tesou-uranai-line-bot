...
- ユーザーがLINEのトーク履歴を削除しても、サーバー側のセッション（Redis）自体は消えません。過去の鑑定内容が画面から見えなくなって混乱しないよう、ラリーの返信には毎回「新しく占う」ボタン（Flex Message。生のURLテキストではない）を添えており、OpenAI呼び出し自体が失敗した場合も無反応にはせずLIFFへの案内を返すようにしています
