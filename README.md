...
5. サーバー側でsharpにより最終リサイズ（長辺768px・EXIF除去）→ OpenAI Visionで鑑定 → LINE Push Messageで結果を送信（鑑定に成功した場合は、続けて「気になることがあれば質問してください」という案内メッセージと、「新しくする」のクイックリプライも送る）
...
- 鑑定結果のあとの案内メッセージには「このトークにそのまま質問してみてください」と書かれており、そのまま自由にメッセージを入力するとラリーが始まります。加えて「新しくする」のクイックリプライも付いており、タップするとLIFFを直接開けます（「続きを聞く」のような紛らわしいボタンは置いていません。案内文とボタンで同じことを二重に示すと迷う原因になるためです）
...
- 2回目以降の返信にも、まだ質問を続けられることが伝わるよう「他に気になることがあれば、続けて聞いてくださいね。」という固定の一言を毎回添えています（AIに生成させず固定文言にすることで、コストと文言のブレを抑えています）
...
- ユーザーがLINEのトーク履歴を削除しても、サーバー側のセッション（Redis）自体は消えません。ラリーの通常の返信（1〜9往復目）には、目立つボタン付きメッセージ（Flex Message）ではなく、控えめな**クイックリプライ**で「新しくする」を毎回添えています。**10往復目に達した終了メッセージの時だけ**、大きめのボタン付きメッセージ（Flex Message）に切り替わります
