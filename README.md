...
- ユーザーがLINEのトーク履歴を削除しても、サーバー側のセッション（Redis）自体は消えません。ラリーの通常の返信（1〜9往復目）は「終わり？」と紛らわしくならないようテキストのみにし、**10往復目に達した終了メッセージの時だけ**「新しく占う」ボタン（Flex Message）を表示します
- OpenAI呼び出し自体が失敗した場合は、ラリーの途中であっても無反応にはせず、LIFFへの案内（ボタン付き）を返すようにしています
