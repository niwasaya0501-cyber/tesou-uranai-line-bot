...
│   ├── openai.js          # OpenAI Vision呼び出し・プロンプト生成
...
3. 悩み（恋愛／仕事／人間関係／自己分析／その他）を選択し、写真を選ぶ
4. ブラウザ側で軽くリサイズしてから `/api/liff-submit` にPOST
5. サーバー側でsharpにより最終リサイズ（長辺768px・EXIF除去）→ OpenAI Visionで鑑定 → LINE Push Messageで結果を送信
...
- 課金が発生するのはOpenAI APIのみです。Vercel・LINE Messaging APIはいずれも無料枠内で運用できます
