...
// 往復上限に達した際、これまでの鑑定内容・やりとりを消すか残すか選んでもらうためのクイックリプライ。
// メッセージアクションにしているのは、タップ＝そのままユーザー発言として送られ、
// webhook側の「消して」「残す」キーワード判定にそのまま乗せられるため
function buildEndOfRallyQuickReply() {
  return {
    items: [
      {
        type: 'action',
        action: { type: 'message', label: '消す', text: '消して' },
      },
      {
        type: 'action',
        action: { type: 'message', label: '残す', text: '残す' },
      },
    ],
  };
}
