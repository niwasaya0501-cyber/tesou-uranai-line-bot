...
// 出力してしまうことがあるため、画面にプレーンテキストとして表示する前に念のため除去する
...
// 鑑定結果の下のチャット欄に「例」として示す文言。選んだ悩みとズレた例
...
function getFollowUpExample(worryKey, worryText) {
  if (worryKey === 'other' && worryText) return `${worryText}についてもっと詳しく教えて`;
  return FOLLOW_UP_EXAMPLES[worryKey] || '今年の運勢は？';
}
...
"reading"の中身は画面にそのまま表示されるプレーンテキストです。**太字**や見出し（#）、箇条書きの記号（*, -）などのMarkdown記法は使わず、装飾のない文章だけで書いてください。`;
}
...
# 出力形式について
これは画面にそのまま表示されるプレーンテキストです。**太字**や見出し（#）、箇条書きの記号（*, -）などのMarkdown記法は使わず、装飾のない文章だけで書いてください。`;
}
...
module.exports = { readPalm, continueChat, WORRY_LABELS, getFollowUpExample };
